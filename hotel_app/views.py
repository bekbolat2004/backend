...
from .models import Hotels, Rooms, Reservation,UserProfile
...
from .models import Hotels,Rooms,Reservation, UserProfile, User
from django.contrib import messages
from django.contrib.auth import authenticate,login,logout
from django.contrib.auth.models import User
from django.contrib.auth.decorators import login_required
...
def homepage(request):
    ...
    if request.method == "POST":
        try:
            ...
            room = Rooms.objects.all().filter(hotel=hotel, capacity__gte=int(request.POST['capacity'])).exclude(
                id__in=rr)
            if len(room) == 0:
...
def panel(request):
    ...
    response = render(request, 'staff/panel.html',
                      {'location': hotel, 'reserved': reserved, 'rooms': rooms, 'total_rooms': total_rooms,
